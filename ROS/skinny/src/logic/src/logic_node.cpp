...
void joystickAxisCallback(const messages::AxisState& axisState){
    std::cout << "Button " << axisState.joystick << " " << axisState.axis << " " << axisState.state << std::endl;
    if(axisState.axis==0){
...
void joystickButtonCallback(const messages::ButtonState& buttonState){
...
void joystickHatCallback(const messages::HatState& hatState){
...
//	ros::Subscriber joystick1ThrottleSubscriber=nodeHandle.subscribe("joystick_1_throttle",1,joystick1ThrottleCallback);
    ros::Subscriber joystickAxisSubscriber=nodeHandle.subscribe("joystick_axis",1,joystickAxisCallback);
	ros::Subscriber joystickButtonSubscriber=nodeHandle.subscribe("joystick_button",1,joystickButtonCallback);
	ros::Subscriber joystickHatSubscriber=nodeHandle.subscribe("joystick_hat",1,joystickHatCallback);
